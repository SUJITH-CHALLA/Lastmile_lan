...
export default function ResumeAnalyzer() {
  const [phase, setPhase] = useState("upload"); // upload | processing | animating | complete
  ...
  const onDrop = useCallback((e) => {
    ...
    if (file && (file.type === "text/plain" || file.name.endsWith(".txt"))) {
      ...
      setError("Please upload a .txt file or paste resume text directly.");
    }
  }, [handleTextFile]);

  // Calls your Next.js API route — key stays on the server
  const runAnalysis = async () => {
    if (!resumeText.trim()) return;
    setPhase("processing");
    setError(null);
    setLogs([{ id: 0, type: "info", msg: "Initializing LASTMILE engine...", ts: ts() }]);
    setProgress(5);

    try {
      setLogs(p => [...p, { id: 1, type: "info", msg: `Routing to ${CEREBRAS_MODEL}...`, ts: ts() }]);
      setProgress(15);

      // Calls /api/analyze-resume — your Next.js route handles the Cerebras key
  ...
  const startLogAnimation = (data) => {
    ...
    setLogs([{ id: -1, type: "ok", msg: "Inference complete — replaying audit log...", ts: ts() }]);
    ...
    return (
      <div style={{
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 8 }}>
          ...
          {scanning && <span style={{ background: Y, color: BK, fontSize: "7px", fontWeight: 700, padding: "1px 6px" }}>◉ SCANNING</span>}
        ...
        {allIssues.map((f, i) => (
          <div key={i} style={{ border: `2px solid ${sevCols[f.severity]||"#94a3b8"}`, padding: "10px 14px", marginBottom: 8, background: "#fff" }}>
            <div style={{ display: "flex", gap: 8, alignItems: "center", marginBottom: 6 }}>
              <span style={{ background: sevCols[f.severity]||"#94a3b8", color:"#fff", fontSize:"8px", fontWeight:700, padding:"1px 6px" }}>{(f.severity||"low").toUpperCase()}</span>
              <span style={{ fontSize: "9px", color: "#6b7280" }}>{f.section}</span>
            </div>
    ...
    return (
      <div style={{ border: `3px solid ${BK}`, background: "#fff", maxWidth: 700, margin: "0 auto" }}>
        ...
        <div style={{ padding: "12px 18px", background: "#f9fafb", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <span style={{ fontSize: "8px", color: "#9ca3af", letterSpacing: "2px" }}>GENERATED BY LASTMILE AI ENGINE V2</span>
  ...
  if (phase === "upload") return (
    <div style={{ fontFamily: mono, background: "#f0ede8", minHeight: "100vh", display: "flex", flexDirection: "column" }}>
      ...
      <div style={{ background: BK, borderBottom: `3px solid ${Y}`, padding: "10px 20px", display: "flex", alignItems: "center", gap: 12 }}>
        ...
        <span style={{ flex: 1 }} />
        <span style={{ color: "#4ade80", fontSize: "8px", letterSpacing: "2px" }}>● {CEREBRAS_MODEL}</span>
      </div>
  ...
  return (
    <div style={{ fontFamily: mono, background: "#f0ede8", height: "100vh", display: "flex", flexDirection: "column", overflow: "hidden" }}>
      ...
      <div style={{ background: BK, borderBottom: `3px solid ${Y}`, padding: "9px 18px", display: "flex", alignItems: "center", gap: 14, flexShrink: 0 }}>
        <span style={{ color: Y, fontWeight: 700, fontSize: "14px", letterSpacing: "3px" }}>LASTMILE</span>
        <span style={{ background: Y, color: BK, fontSize: "8px", fontWeight: 700, padding: "2px 8px", letterSpacing: "2px" }}>EVOLUTION 2.1</span>
        <span style={{ background: "#1f1f1f", color: "#4ade80", fontSize: "8px", fontWeight: 700, padding: "2px 8px" }}>{CEREBRAS_MODEL}</span>
        <span style={{ flex: 1 }} />
        ...
        <span style={{ color: "#6b7280", fontSize: "9px" }}>
          {phase==="processing" ? "CALLING API..." : phase==="animating" ? "ANALYZING..." : "COMPLETE"}
        </span>
      ...
      <div style={{ background: BK, borderTop: `3px solid ${Y}`, padding: "9px 18px", display: "flex", alignItems: "center", gap: 14, flexShrink: 0 }}>
        <span style={{ color: "#6b7280", fontSize: "8px", letterSpacing: "2px", minWidth: 100 }}>
